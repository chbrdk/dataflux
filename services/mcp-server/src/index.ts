...
const SearchRequestSchema = z.object({
  ...
  confidence_min: z.number().min(0).max(1).optional().default(0.7),
});
...
const AnalyzeRequestSchema = z.object({
  entity_id: z.string().min(1, 'Entity ID cannot be empty'),
  analysis_type: z.enum(['summary', 'objects', 'scenes', 'transcript', 'full']).optional().default('summary'),
...
const tools = {
  ...
  dataflux_upload: {
    name: 'dataflux_upload',
    description: 'Upload media files to DataFlux for processing',
    inputSchema: {
      type: 'object',
      properties: {
        file_path: {
        ...
        context: {
          type: 'string',
          description: 'Optional context or description for the file',
        },
        priority: {
          type: 'number',
          description: 'Processing priority (1=high, 10=low)',
          default: 5,
        },
        collection_id: {
          type: 'string',
          description: 'Optional collection ID to group related files',
        },
      },
      required: ['file_path'],
    },
...
// Tool handlers
async function handleDatafluxSearch(args: any) {
  try {
...
async function handleDatafluxAnalyze(args: any) {
  try {
...
async function handleDatafluxSimilar(args: any) {
  try {
...
async function handleDatafluxUpload(args: any) {
  try {
    const { file_path, context, priority = 5, collection_id } = args;
    
    // For now, return a mock response since file upload requires file handling
    return {
      content: [
        {
          type: 'text',
          text: `File upload simulation for ${file_path}:\n- Context: ${context || 'None'}\n- Priority: ${priority}\n- Collection: ${collection_id || 'Default'}\n\nNote: Actual file upload requires file system access.`,
        },
      ],
    };
  } catch (error) {
...
// Prompt handlers
async function getAnalyzeVideoPrompt(args: any) {
...
async function getSearchInsightsPrompt(args: any) {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case 'dataflux_upload':
      return await handleDatafluxUpload(args);
    default:
...
// Transport selection (STDIO only for now)
